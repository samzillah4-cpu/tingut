        ...
        <div class="col-md-12">
            @if(session('success'))
                <div class="alert alert-success alert-dismissible">
                    ...
                    {{ session('success') }}
                    @if(session('refresh_frontend'))
                        <br><small><a href="{{ url('/') }}?refresh=1" target="_blank" class="alert-link">Click here to view the updated frontend</a></small>
                    @endif
                </div>
            @endif
        ...
        <div class="col-lg-6 col-md-12">
            ...
            <div class="card">
                ...
                <div class="card-body" style="display: none;">
                    <form id="menuForm" action="{{ route('admin.website.menus.update') }}" method="POST">
                        @csrf
                        <div id="menuItems">
                            @foreach($menus as $menu)
                             <div class="menu-item mb-3 p-3 border rounded" data-id="{{ $menu->id }}">
                                 <input type="hidden" name="menus[{{ $menu->id }}][id]" value="{{ $menu->id }}">
                                 <div class="row">
                                     <div class="col-md-4">
                                         <div class="form-group">
                                             <label>Menu Name</label>
                                             <input type="text" class="form-control form-control-sm" name="menus[{{ $menu->id }}][name]" value="{{ $menu->name }}" required>
                                         </div>
                                     </div>
                                    <div class="col-md-3">
                                        <div class="form-group">
                                            <label>Link to Page</label>
                                            <select class="form-control form-control-sm menu-page-select" data-menu-id="{{ $menu->id }}">
                                                <option value="">Select a page...</option>
                                                @foreach($allPages as $page)
                                                    <option value="{{ $page['url'] }}" {{ $menu->url == $page['url'] ? 'selected' : '' }}>{{ $page['title'] }}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                    </div>
                                    <div class="col-md-3">
                                        <div class="form-group">
                                            <label>URL</label>
                                            <input type="text" class="form-control form-control-sm menu-url-input" name="menus[{{ $menu->id }}][url]" value="{{ $menu->url }}" placeholder="/" data-menu-id="{{ $menu->id }}">
                                        </div>
                                    </div>
                                    <div class="col-md-2">
                                        <div class="form-group">
                                            <label>Order</label>
                                            <input type="number" class="form-control form-control-sm" name="menus[{{ $menu->id }}][order]" value="{{ $menu->order }}" min="1">
                                        </div>
                                    </div>
                                </div>
                                <div class="row">
                                    <div class="col-md-6">
                                        <div class="form-check">
                                            <input type="checkbox" class="form-check-input" name="menus[{{ $menu->id }}][is_active]" value="1" {{ $menu->is_active ? 'checked' : '' }}>
                                            <label class="form-check-label">Active</label>
                                        </div>
                                    </div>
                                    <div class="col-md-6">
                                        <div class="form-check">
                                            <input type="checkbox" class="form-check-input" name="menus[{{ $menu->id }}][open_in_new_tab]" value="1" {{ $menu->open_in_new_tab ? 'checked' : '' }}>
                                ...
                                <div class="row mt-2">
                                    <div class="col-12">
                                        <button type="button" class="btn btn-danger btn-sm delete-menu" data-id="{{ $menu->id }}">
                    ...
                    <form action="{{ route('admin.hero.update') }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control" id="heading" name="heading"
                                   value="{{ $hero->heading ?? 'Welcome to TingUt.no' }}" required>
                        </div>

                        <div class="form-group">
                            <label for="paragraph">Paragraph Text</label>
                            <textarea class="form-control" id="paragraph" name="paragraph" rows="3">{{ $hero->paragraph ?? '' }}</textarea>
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            @if($hero && $hero->background_image)
                                <div class="mt-2">
                                    <img src="{{ asset('storage/' . $hero->background_image) }}" alt="Current Hero Image" style="max-height: 150px; max-width: 100%;">
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <input type="text" class="form-control" id="button1_text" name="button1_text"
                                           value="{{ $hero->button1_text ?? 'Browse Products' }}" required>
                                </div>
                            </div>
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <input type="text" class="form-control" id="button1_url" name="button1_url"
                                           value="{{ $hero->button1_url ?? '/products' }}" required>
                                </div>
                            </div>
                        </div>
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <input type="text" class="form-control" id="button2_text" name="button2_text"
                                           value="{{ $hero->button2_text ?? 'Become a Seller' }}" required>
                                </div>
                            </div>
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <input type="text" class="form-control" id="button2_url" name="button2_url"
                                           value="{{ $hero->button2_url ?? '/register' }}" required>
                                </div>
                            </div>
                        </div>

                        <div class="form-check">
                            <input type="checkbox" class="form-check-input" id="is_active" name="is_active" value="1" {{ ($hero && $hero->is_active) ? 'checked' : 'checked' }}>
                        ...
                        <div class="mt-3">
                            ...
                            <a href="{{ url('/') }}" target="_blank" class="btn btn-info ml-2">
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-8">
                            ...
                            <div class="row">
                                ...
                                @endphp
                                @foreach($standardPages as $pageKey)
                                    @php
                                    ...
                                    <div class="col-md-6 mb-3">
                                        <div class="card border">
                                            <div class="card-body p-3">
                                                <h6 class="card-title mb-2">{{ $pageLabels[$pageKey] ?? ucfirst($pageKey) }}</h6>
                                                @if($page)
                                                    <p class="card-text small text-muted mb-2">{{ Str::limit($page->content, 50) }}</p>
                                                    <div class="d-flex gap-1">
                                                        <a href="{{ route('admin.pages.edit', $page) }}" class="btn btn-outline-primary btn-sm">
                                                            <i class="fas fa-edit"></i> Edit
                                                        </a>
                                                        <a href="{{ route('pages.show', $page->slug) }}" target="_blank" class="btn btn-outline-info btn-sm">
                                                            <i class="fas fa-eye"></i> View
                                                        </a>
                                                    </div>
                                                @else
                                                    <p class="card-text small text-muted mb-2">Page not created yet</p>
                                                    <button type="button" class="btn btn-outline-success btn-sm" onclick="createStandardPage('{{ $pageKey }}', '{{ $pageLabels[$pageKey] }}')">
        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header" data-widget="collapse">
                    ...
                    <div class="card-tools">
                        <a href="{{ route('admin.testimonials') }}" class="btn btn-success btn-sm">
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6">
                            ...
                            <div class="d-flex flex-column gap-2">
                                <a href="{{ route('admin.testimonials') }}" class="btn btn-outline-primary btn-sm">
                                    <i class="fas fa-list"></i> View All Testimonials
                                </a>
                                <a href="{{ route('admin.testimonials.create') }}" class="btn btn-outline-success btn-sm">
                                    <i class="fas fa-plus"></i> Create New Testimonial
                                </a>
                                <a href="{{ url('/') }}" target="_blank" class="btn btn-outline-info btn-sm">
    ...
    <div class="modal fade" id="addMenuModal" tabindex="-1" role="dialog" aria-labelledby="addMenuModalLabel" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <form action="{{ route('admin.website.menus.create') }}" method="POST">
                    @csrf
                    <div class="modal-body">
                        <div class="form-group">
...
$(document).ready(function() {
    ...
    $('.delete-menu').on('click', function() {
        ...
        if (confirm('Are you sure you want to delete this menu item?')) {
            $.ajax({
                url: '{{ url("admin/website/menus") }}/' + menuId,
                type: 'DELETE',
                data: {
                    _token: '{{ csrf_token() }}'
                },
