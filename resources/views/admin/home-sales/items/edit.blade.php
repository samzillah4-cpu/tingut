        ...
        <div class="card-header bg-white border-bottom-0 py-3">
            <div class="d-flex align-items-center">
                ...
                <div>
                    ...
                    <small class="text-muted">Update the item details</small>
                </div>
            </div>
        </div>
        <div class="card-body p-4">
            <form action="{{ route('admin.home-sales.items.update', [$homeSale, $item]) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                    ...
                    <div class="col-md-4">
                        <div class="form-group">
                            ...
                            <div class="input-group">
                                ...
                                <input type="number" class="form-control @error('price') is-invalid @enderror" id="price" name="price" value="{{ old('price', $item->price) }}" step="0.01" min="0" placeholder="0.00">
                                @error('price')
                                    <span class="invalid-feedback" role="alert">
                                        <strong>{{ $message }}</strong>
                                    </span>
                ...
                <div class="form-group">
                    ...
                    <textarea class="form-control @error('description') is-invalid @enderror" id="description" name="description" rows="3" placeholder="Describe the item">{{ old('description', $item->description) }}</textarea>
                    @error('description')
                        <span class="invalid-feedback" role="alert">
                            <strong>{{ $message }}</strong>
                        </span>
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <select class="form-control @error('category') is-invalid @enderror" id="category" name="category">
                                ...
                                <option value="Toys" {{ old('category', $item->category) === 'Toys' ? 'selected' : '' }}>Toys</option>
                                <option value="Kitchenware" {{ old('category', $item->category) === 'Kitchenware' ? 'selected' : '' }}>Kitchenware</option>
                                <option value="Garden" {{ old('category', $item->category) === 'Garden' ? 'selected' : '' }}>Garden</option>
                                <option value="Sports" {{ old('category', $item->category) === 'Sports' ? 'selected' : '' }}>Sports</option>
                ...
                <div class="form-group">
                            ...
                            <div class="col-md-3">
                                @if($item->image)
                                    @if(str_starts_with($item->image, 'http'))
                                        <img src="{{ $item->image }}" alt="Item Image" class="img-thumbnail" style="width: 100%; height: 150px; object-fit: cover;">
                                    @else
                                        <img src="{{ asset('storage/' . $item->image) }}" alt="Item Image" class="img-thumbnail" style="width: 100%; height: 150px; object-fit: cover;">
                                    @endif
                                @endif
                            </div>
                        </div>
                    ...
                    <small class="form-text text-muted">Upload a new image to replace the existing one (max 2MB)</small>
                    @error('image')
...
@section('js')
    <script>
        ...
        document.getElementById('image').addEventListener('change', function(e) {
            var fileName = e.target.files.length > 0 ? e.target.files[0].name : 'Choose new file';
