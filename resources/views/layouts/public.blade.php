...
<head>
    ...
    <!-- Basic Meta Tags -->
    <meta name="description" content="{{ config('settings.site_description', 'Exchange and trade platform for buying, selling, and renting items') }}">
    <meta name="keywords" content="exchange, trade, buy, sell, rent, {{ config('settings.site_name', 'Bytte.no') }}">
    ...
    <meta property="og:title" content="{{ config('settings.site_name', 'Bytte.no') }}">
    <meta property="og:description" content="{{ config('settings.site_description', 'Exchange and trade platform for buying, selling, and renting items') }}">
    <meta property="og:url" content="{{ url()->current() }}">
    <meta property="og:locale" content="en_US">

    <!-- Twitter Card (Default) -->
    <meta name="twitter:card" content="summary_large_image">
    <meta name="twitter:title" content="{{ config('settings.site_name', 'Bytte.no') }}">
    <meta name="twitter:description" content="{{ config('settings.site_description', 'Exchange and trade platform for buying, selling, and renting items') }}">
...
<header class="finn-header">
    <div class="header-main">
        ...
        <div class="header-logo">
            <a href="/">
                @if(config('settings.site_logo'))
                    <img src="{{ asset('storage/' . config('settings.site_logo')) }}" alt="{{ config('settings.site_name', 'TingUt.no') }}">
                @endif
                <span>{{ config('settings.site_name', 'TingUt.no') }}</span>
            </a>
        </div>

        <!-- Desktop Search Bar -->
        <div class="header-search header-search-desktop">
            <form action="{{ route('products.index') }}" method="GET" class="search-form" id="desktopSearchForm">
                <div class="search-input-wrapper" style="position: relative;">
                    <i class="fas fa-search search-icon"></i>
                    <input type="text" name="search" id="desktopSearchInput" class="search-input" placeholder="Search for items..." value="{{ request('search') }}" autocomplete="off">
        ...
        <div class="header-actions">
            ...
            <a href="{{ route('messages.inbox') }}" class="header-action-btn">
            ...
            @auth
                <div class="dropdown">
                    ...
                    <ul class="dropdown-menu dropdown-menu-end" aria-labelledby="userDropdown" style="border: none; border-radius: 8px; box-shadow: 0 4px 20px rgba(0,0,0,0.12); margin-top: 8px; min-width: 180px;">
                        <li><a class="dropdown-item" href="{{ route('dashboard') }}" style="padding: 10px 16px; border-radius: 4px;"><i class="fas fa-tachometer-alt me-2"></i>Dashboard</a></li>
                        <li><a class="dropdown-item" href="{{ route('profile.edit') }}" style="padding: 10px 16px; border-radius: 4px;"><i class="fas fa-user-edit me-2"></i>Profile</a></li>
        ...
        <div class="nav-container">
            ...
            <div class="nav-items">
                ...
                @foreach($menus as $menu)
                    @if(strtolower($menu->name) !== 'products' && strtolower($menu->name) !== 'categories' && strtolower($menu->name) !== 'about')
                        <a href="{{ $menu->url }}" @if($menu->open_in_new_tab) target="_blank" @endif
                           class="nav-item {{ request()->is(trim($menu->url, '/')) ? 'active' : '' }}">
                            @if(strtolower($menu->name) === 'about' || strtolower($menu->name) === 'about us')
                                <i class="fas fa-info-circle"></i>
                            @elseif(strtolower($menu->name) === 'contact' || strtolower($menu->name) === 'contact us')
                                <i class="fas fa-envelope"></i>
                            @elseif(strtolower($menu->name) === 'blog')
                                <i class="fas fa-blog"></i>
                            @elseif(strtolower($menu->name) === 'services' || strtolower($menu->name) === 'service')
                                <i class="fas fa-cogs"></i>
                            @elseif(strtolower($menu->name) === 'faq' || strtolower($menu->name) === 'faqs')
                                <i class="fas fa-question-circle"></i>
                            @elseif(strtolower($menu->name) === 'help')
                                <i class="fas fa-life-ring"></i>
                            @elseif(strtolower($menu->name) === 'privacy' || strtolower($menu->name) === 'privacy policy')
                                <i class="fas fa-shield-alt"></i>
                            @elseif(strtolower($menu->name) === 'terms' || strtolower($menu->name) === 'terms of service')
                ...
                <a href="{{ route('home-sales.index') }}" class="nav-item {{ request()->routeIs('home-sales.*') ? 'active' : '' }}">
...
<div class="header-search" id="mobileSearch">
    ...
    <form action="{{ route('products.index') }}" method="GET" class="mobile-search-form" id="mobileSearchForm">
        <div class="mobile-search-input-wrapper" style="position: relative;">
            <i class="fas fa-search mobile-search-icon-input"></i>
            <input type="text" name="search" id="mobileSearchInput" class="mobile-search-input" placeholder="Search for items..." value="{{ request('search') }}" autofocus autocomplete="off">
    ...
    <section class="hero-section" style="{{ $heroBackgroundStyle }}">
        <div class="container">
            <div class="hero-content">
                ...
                <p class="hero-text">{{ $hero->paragraph }}</p>
                <div>
                    @if($hero->button1_text && $hero->button1_url)
                        <a href="{{ $hero->button1_url }}" class="btn-primary-finn" style="display: inline-block; margin: 0 10px 10px 0;">{{ $hero->button1_text }}</a>
                    @endif
                    @if($hero->button2_text && $hero->button2_url)
                        <a href="{{ $hero->button2_url }}" class="btn-outline-finn" style="display: inline-block; border-color: var(--primary-color); color: var(--primary-color);">{{ $hero->button2_text }}</a>
                    @endif
                </div>
            </div>
        </div>
...
@include('components.gdpr-banner')
...
<footer>
        ...
        <div class="footer-grid">
            ...
            <div>
                <div class="footer-title">
                    @if(config('settings.site_logo'))
                        <img src="{{ asset('storage/' . config('settings.site_logo')) }}" alt="{{ config('settings.site_name', 'TingUt.no') }}" height="40" class="mb-3" style="border-radius: 8px;">
                    @else
                        <h5 class="fw-bold">{{ config('settings.site_name', 'TingUt.no') }}</h5>
                    @endif
                </div>
                <p style="font-size: 14px; color: var(--finn-text-light); margin-bottom: 16px;">
                    {{ config('settings.footer_description', 'Exchange items, reduce waste, and build connections in your local community.') }}
                </p>
                @if(config('settings.contact_email'))
                    <div class="mb-2">
                        <i class="fas fa-envelope me-2" style="color: var(--finn-blue);"></i>
                        <a href="mailto:{{ config('settings.contact_email') }}" class="footer-links" style="display: inline;">{{ config('settings.contact_email') }}</a>
                    </div>
                @endif
                @if(config('settings.contact_phone'))
                    <div>
                        <i class="fas fa-phone me-2" style="color: var(--finn-blue);"></i>
                        <span style="color: var(--finn-text-light);">{{ config('settings.contact_phone') }}</span>
                    </div>
                @endif
            </div>
            ...
            <div>
                ...
                <ul class="footer-links">
                    ...
                    <li><a href="{{ route('products.index') }}">Browse Items</a></li>
            ...
            <div>
                ...
                <ul class="footer-links">
                    @auth
                        ...
                        <li><a href="{{ route('subscriptions') }}">My Subscriptions</a></li>
                    @else
            ...
            <div>
                ...
                <ul class="footer-links">
                    @if(config('settings.privacy_policy_url'))
                        <li><a href="{{ config('settings.privacy_policy_url') }}">Privacy Policy</a></li>
                    @endif
                    @if(config('settings.terms_of_service_url'))
                        <li><a href="{{ config('settings.terms_of_service_url') }}">Terms of Service</a></li>
                    @endif
                    @if(config('settings.contact_page_url'))
                        <li><a href="{{ config('settings.contact_page_url') }}">Contact Us</a></li>
                    @endif
                </ul>
            </div>
            </div>
    ...
    <div class="footer-bottom">
        <p class="mb-2">
            © {{ date('Y') }} TingUt.no. All rights reserved.
        </p>
        ...
        function performSearch(query, resultsContainer) {
            fetch(`{{ route('api.search') }}?q=${encodeURIComponent(query)}`)
                ...
                .then(data => {
                    ...
                    // View all link
                    html += `<a href="{{ route('products.index') }}?search=${encodeURIComponent(query)}" class="search-view-all">View all results</a>`;
